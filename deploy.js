...
const DEPLOY_README = [
  ...
  '    # 数据库通过 .sql 导入（见下方「数据导入」），无需跑 init/seed',
  ...
  '## 数据导入',
  '',
  '建表 + 业务数据 + 登录账号均通过 .sql 一次性导入：',
  '',
  '    mysql -u root -p < server/src/db/wood_store_data.sql',
  '',
  '该 .sql 含 DROP+CREATE+INSERT，会干净恢复整库。生成方式见 server/src/db/export-data.js。',
  '若只需补登录账号（空库场景）：npm run db:seed-users',
  '',
